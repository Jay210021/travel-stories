...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data, error }) => {
      if (data.user?.email && authors.has(data.user.email.toLowerCase())) { setStatus(`已登入：${data.user.email}`); setMode("editor"); }
      else if (data.user) { setStatus("這個 Google 帳號不在作者名單中。"); supabase.auth.signOut(); }
      else setStatus(error ? "登入狀態確認失敗，請再試一次" : "尚未登入");
    });
  }, []);
