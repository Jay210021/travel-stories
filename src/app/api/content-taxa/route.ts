...
export async function GET() {
  ...
  return error ? NextResponse.json({ error: error.message }, { status: 400 }) : NextResponse.json({ taxa: data ?? [] });
}
...
export async function POST(request: Request) {
  ...
  const parentId = parentResult.parent?.id ?? null; const order = await nextOrder(author, parentId); if (order.error) return NextResponse.json({ error: order.error.message }, { status: 400 });
  ...
  return error ? NextResponse.json({ error: error.code === "23505" ? "同一層已經有相同名稱的分類。" : error.message }, { status: 400 }) : NextResponse.json({ taxon: data });
}
...
export async function PATCH(request: Request) {
  ...
  if (currentError || !current) return NextResponse.json({ error: currentError?.message || "找不到分類。" }, { status: 404 });
  ...
  if (current.kind !== "system") {
    ...
    if (update.parent_id !== current.parent_id) { const order = await nextOrder(author, update.parent_id as string | null); if (order.error) return NextResponse.json({ error: order.error.message }, { status: 400 }); update.sort_order = order.value; }
  }
